        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">Detail Data Laporan Akhir</h1>
                    <!-- <div>
                ...
                <div class="col-12">
                    ...
                    <div class="card-header py-3 text-white bg-danger border-danger">
                        <h5 class="my-0 fw-normal">File Lapoan Akhir</h5>
                    </div>
                    <div class="card-body">
                        <h4 class="card-title pricing-card-title"><small class="text-muted fw-light">{{ lapakhirs.fl_lapakhir }}</small></h4>
                        <a :href="`http://localhost:8000/storage/LapAkhirs/${lapakhirs.fl_lapakhir}`" class="btn btn-md btn-danger"><i class="bi bi-file-pdf"></i> Download File</a>
                    </div>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-sm">
                        ...
                        <tbody>
                            <tr>
                                <td>{{ lapakhirs.judul }}</td>
                                <td>{{ lapakhirs.catatan }}</td>
                                <td>{{ lapakhirs.nip }}</td>
                                <td>{{ moment(String(lapakhirs.created_at)).format('dddd-MM-YYYY') }}</td>
                                <!-- <td><router-link :to="{ name: 'admin.proposal_detail', params: {id: prop.id} }" class="btn btn-sm btn-warning">Lihat Data</router-link></td> -->
                            </tr>
                        </tbody>
                    </table>
                    <div v-if="lapakhirs.status == 'Sedang diproses'">
...
export default {
    setup() {
        ...
        function terima() {
            axios.put(`http://localhost:8000/api/editstatuslapakhir/${route.params.id}`, {
                status: 'Diterima'
            }).then(response => {
