    ...
    <header class="navbar navbar-dark sticky-top bg-dark flex-md-nowrap p-0 shadow">
        ...
        <div class="row">
            ...
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                <h1 class="h2">Detail Data Luaran</h1>
            ...
            <div class="col-12">
                    ...
                    <div class="card-header py-3 text-white bg-danger border-danger">
                        <h5 class="my-0 fw-normal">File Luaran</h5>
                    </div>
                    <div class="card-body">
                        <h4 class="card-title pricing-card-title"><small class="text-muted fw-light">{{ luarans.fl_luaran }}</small></h4>
                        <a :href="`http://localhost:8000/storage/luarans/${luarans.fl_luaran}`" class="btn btn-md btn-danger"><i class="bi bi-file-pdf"></i> Download File</a>
                    </div>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-sm">
                        ...
                        <tbody>
                            <tr>
                                <td>{{ luarans.judul }}</td>
                                <td><a :href="luarans.publikasi">{{ luarans.publikasi }}</a></td>
                                <td><a :href="luarans.artikel">{{ luarans.artikel }}</a></td>
                                <td>{{ moment(String(luarans.created_at)).format('dddd-MM-YYYY') }}</td>
                                <td>{{ luarans.nip }}</td>
                                <!-- <td><router-link :to="{ name: 'admin.proposal_detail', params: {id: prop.id} }" class="btn btn-sm btn-warning">Lihat Data</router-link></td> -->
                            </tr>
                        </tbody>
                    </table>
                    <div v-if="luarans.status == 'Sedang diproses'">
                        <a @click.prevent="terima" class="btn btn-md btn-success"><i class="bi bi-check-lg"></i> Terima Luaran</a>
                    </div>
                    <div v-else>
...
export default {
    setup() {
        ...
        function terima() {
            axios.put(`http://localhost:8000/api/editstatusluaran/${route.params.id}`, {
                status: 'Diterima'
            }).then(response => {
                alert('Luaran diterima')
