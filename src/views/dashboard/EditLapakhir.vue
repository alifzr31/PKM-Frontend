            ...
            <header>
                <div class="d-flex flex-column flex-md-row align-items-center pb-3 mb-4 border-bottom">
                    ...
                    <nav class="d-inline-flex mt-2 mt-md-0 ms-md-auto">
                        ...
                        <b style="color: #0d6efd;">Laporan</b>
                        </a>
                        <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                            <li><router-link :to="{ name: 'lapkem.index'}" class="dropdown-item">Laporan Kemajuan</router-link></li>
                            <li><router-link :to="{ name: 'lapakhir.index'}" class="dropdown-item"><b style="color: #0d6efd;">Laporan Akhir</b></router-link></li>
                            <!-- <li><hr class="dropdown-divider"></li>
                            <li><a class="dropdown-item" href="#">Something else here</a></li> -->
                        </ul>
                        <router-link :to="{ name: 'luaran.index'}" class="me-3 py-2 text-dark text-decoration-none">Luaran</router-link>
                    </nav>
                ...
                <div>
                    ...
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><router-link :to="{ name: 'lapakhir.index' }" class="text-decoration-none">Laporan Akhir</router-link></li>
                        <li class="breadcrumb-item active" aria-current="page"><router-link :to="{ name: 'lapakhir.detail' }" class="text-decoration-none">{{ jdl.judul }}</router-link></li>
                ...
                <h1 class="display-4 fw-normal">{{ jdl.judul }}</h1>
                </div>
            </header>
                ...
                <div class="col-md-7 col-lg-12">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-6">
                            ...
                            <select v-model="lapakhirs.judul" class="form-control">
                        ...
                        <div class="col-6">
                        <label for="address2" class="form-label">File Laporan Akhir <span class="text-muted">(Optional)</span> <i><span style="color: red;"> *.pdf, .docx, .doc | Max 2048Kb</span></i></label>
                        <input type="file" class="form-control" @change="selectLapakhir">
                        </div>

                        <div class="col-12">
                        <label for="address" class="form-label">Catatan</label>
                        <textarea class="form-control" placeholder="Catatan" v-model="lapakhirs.catatan" style="height: 200px; resize: none;"></textarea>
...
export default {
    setup() {
        ...
        onMounted(() => {
            ...
            axios.get(`http://localhost:8000/api/lapakhirs/${route.params.id}`)
            .then(response => {
                // console.log(response.data.data)
                jdl.value = response.data.data
                lapakhirs.judul = response.data.data.judul
                lapakhirs.fl_lapakhir = response.data.data.fl_lapakhir
                lapakhirs.catatan = response.data.data.catatan
                lapakhirs.nip = response.data.data.nip
            }).catch(error => {
                console.log(error.response.data);
            })
        })

        function selectLapakhir(event) {
            let selectedFile = event.target.files[0]

            lapakhirs.fl_lapakhir = selectedFile
            console.log(lapakhirs.fl_lapakhir)
        }

        function update() {
            let formData = new FormData()
            formData.append('judul', lapakhirs.judul)
            formData.append('fl_lapakhir', lapakhirs.fl_lapakhir)
            formData.append('catatan', lapakhirs.catatan)
            formData.append('nip', lapakhirs.nip)

            axios.post(`http://localhost:8000/api/lapakhirs/${route.params.id}`, formData)
            .then(response => {
                console.log(response.data.data);
                router.push({
                    name: 'lapakhir.index'
