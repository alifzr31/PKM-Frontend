    ...
    <div>
        ...
        <div class="container py-3">
            <header>
                ...
                <h1 class="display-4 fw-normal">Data Hasil Review Pengajuan Anda</h1><br/>
            ...
            <main>
                ...
                <div class="col" v-for="(rev, index) in reviews" :key="index">
                    <div class="card mb-4 rounded-3 shadow-sm">
                    <div class="card-header py-3">
                        <h4 class="my-0 fw-normal">{{ rev.judul }}</h4>
                    </div>
                    <div class="card-body">
                        <h1 class="card-title pricing-card-title">{{ rev.catatan }}</h1>
                        <ul class="list-unstyled mt-3 mb-4">
                        <li>{{ rev.no_surat }}</li>
                        <li>{{ rev.dana }}</li>
                        </ul>
                        <router-link :to="{ name: 'review.detail', params: {id: rev.id} }" class="w-100 btn btn-lg btn-primary">Lihat Data</router-link>
                    </div>
                    </div>
                </div>
...
export default {
    setup() {
        ...
        function reviewbynip() {
            const user = JSON.parse(localStorage.getItem('data'))

            axios.defaults.headers.common.Authorization = `Bearer ${token}`
            axios.get(`http://localhost:8000/api/reviewsbynip/${user.nip}`)
            .then(response => {
